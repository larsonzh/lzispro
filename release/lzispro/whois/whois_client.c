...
int validate_config() {
    ...
    if (g_config.buffer_size == 0) {  // 改为检查0，因为size_t是无符号的
    ...
    if (g_config.dns_cache_size == 0) {  // 改为检查0
        fprintf(stderr, "Error: Invalid DNS cache size in config\n");
        return 0;
    }
    if (g_config.connection_cache_size == 0) {  // 改为检查0
...
size_t get_free_memory() {  // 改为返回 size_t
...
int validate_cache_sizes() {
    ...
    if (required_mem > free_mem * 1024) {  // free_mem是以KB为单位
        fprintf(stderr,
...
void set_cached_connection(const char *host, int port, int sockfd) {
    ...
    for (int i = 0; i < allocated_connection_cache_size;
         i++) {  // 使用 allocated_connection_cache_size
...
int needs_redirect(const char *response) {
    ...
    for (int i = 0; redirect_flags[i] != NULL; i++) {
        if (strstr(response, redirect_flags[i])) {
            if (g_config.debug && i < 10) {  // 只输出前10个匹配的标记
...
char *perform_whois_query(const char *target, int port, const char *query) {
    ...
    while (redirect_count <= MAX_REDIRECTS) {
        ...
        while (retry_count < g_config.max_retries) {
            ...
            if (connect_with_fallback(current_target, current_port, &sockfd) ==
                0) {
        ...
        if (needs_redirect(result)) {
            ...
            if (redirect_server) {
                ...
                if (strcmp(redirect_server, current_target) == 0) {
                    if (g_config.debug)
                        printf(
                            "[DEBUG] Redirect server same as current target, "
                            "stopping "
                            "redirect\n");
                    ...
                    if (combined_result == NULL) {
                        ...
                        size_t new_len =
                            strlen(combined_result) + strlen(result) + 100;
                ...
                if (!current_target) {
                    if (g_config.debug)
                        printf(
                            "[DEBUG] Memory allocation failed for redirect "
                            "target\n");
                    break;
                }
                ...
                if (g_config.debug)
                    printf(
                        "[DEBUG] No redirect server found, stopping "
                        "redirect\n");
                if (combined_result == NULL) {
                    combined_result = result;
                } else {
